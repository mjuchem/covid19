...
t_states_covidtracking <- t_states_covidtracking.raw %>%
  ...
  mutate(
    tests.per.population =
      ifelse(is.na(tests.per.population) | is.infinite(tests.per.population),
             0,
             tests.per.population
             ),
    positives.per.population =
      ifelse(is.na(positive.per.population) | is.infinite(positive.per.population),
             0,
             positive.per.population
             ),
    deaths.per.population =
      ifelse(is.na(deaths.per.population) | is.infinite(deaths.per.population),
             0,
             deaths.per.population
             )
    )
...
t_us_covidtracking %>%
  ...
  geom_ma(ma_fun = EMA, n = 15,
          size = 2, linetype = "solid", color = "blue", alpha = .3
          ) +
  geom_ma(ma_fun = EMA, n = 30,
          size = 2, linetype = "solid", color = "orange", alpha = .3
          ) +
...
grid.arrange(
  ...
  ggplot(mapping = aes(x = date)) +
    ...
    labs(
      ...
      subtitle = US_RESULTS,
      x = "",
      y = "# Tests/Positives/Deaths (Daily)"
    ),
...
grid.arrange(
  ...
  t_us_covidtracking %>%
    ggplot(mapping = aes(x = date)) +
    geom_line(aes(y = totalTestResults, color = "Tested")) +
    geom_line(aes(y = positive, color = "Tested Positive")) +
    geom_line(aes(y = death, color = "Deaths")) +
    scale_y_continuous(labels = scales::comma) +
    scale_colour_manual(values=c("red", "blue", "orange")) +
    labs(
      title = "US COVID-19 Tested, Positives & Deaths - Cumulative",
      subtitle = US_RESULTS,
      x = "",
      y = "# Tests/Positives/Deaths (Cumulative)",
      caption = COVIDTRACKING_DATA_SOURCE
    ) +
    theme(
      axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
  ...
  t_us_covidtracking %>%
    ...
    scale_colour_manual(values=c("red", "blue", "orange")) +
    labs(
      title = "US COVID-19 Tested Positive & Deaths - Cumulative",
      subtitle = US_RESULTS,
      x = "",
      y = "# Positives/Deaths (Cumulative)",
      caption = COVIDTRACKING_DATA_SOURCE
    ) +
    theme(
      axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
  ...
  t_us_covidtracking %>%
    ...
    scale_colour_manual(values=c("red", "blue", "orange")) +
    labs(
      title = "US COVID-19 Deaths - Cumulative",
      subtitle = US_RESULTS,
      x = "",
      y = "# Deaths (Cumulative)",
      caption = COVIDTRACKING_DATA_SOURCE
...
t_states_usafacts %>%
  ...
  geom_col(fill = "grey", color = "white") +
  geom_ma(ma_fun = EMA, n = 15, size = 2,
          linetype = "solid", color = "blue", alpha = .3
          ) +
  geom_ma(ma_fun = EMA, n = 30, size = 2,
          linetype = "solid", color = "orange", alpha = .3
          ) +
  labs(
