...
t_states_usafacts %>%
  ...
  scale_y_continuous(labels = scales::percent) +
  labs(
    title = "US States COVID-19 Total Deaths Corrected for Population",
    x = "State",
...
grid.arrange(
  ...
  t_us_covidtracking %>%
  ggplot(mapping = aes(x = date)) +
    geom_line(aes(y = negatives.per.tests.ratio, color = "Negatives:Tests Ratio")) +
    scale_y_continuous(labels = scales::percent) +
    theme(
      axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),
    ...
    labs(
      subtitle = paste0("Negatives:Tests Ratio = ",
                        scales::percent(US_TOTAL_NEGATIVE / US_TOTAL_TESTS, accuracy = 0.01)),
      x = "",
      y = ""
    ),
